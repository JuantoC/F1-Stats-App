...
router.get("/", async (req, res) => {
    try {
        let query = "SELECT * FROM car_manufacturer";
...
router.post("/", async (req, res) => {
    ...
    try {
        const [result] = await pool.execute(
            "INSERT INTO car_manufacturer (name, country) VALUES (?, ?)",
            [name, country]
        );
        res.status(201).json({ id: result.insertId, message: "Manufacturer created" });
    } catch (err) {
        console.error(err);
        res.status(500).send("Server error");
    }
...
router.put("/:id", async (req, res) => {
    ...
    try {
        await pool.execute(
            "UPDATE car_manufacturer SET name=?, country=? WHERE id=?",
            [name, country, id]
        );
...
router.delete("/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const [del] = await pool.execute("DELETE FROM car_manufacturer WHERE id=?", [id]);
